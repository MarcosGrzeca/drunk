...
DATABASE <- "icwsm"
clearConsole();
dados <- query("SELECT hashtags FROM tweets t wHERE q1 = 0")
stop_words = tm::stopwords("en")

library(wordcloud)
require(tm)

corp = Corpus(VectorSource(dados$hashtags)) 
...
corpus=Corpus(VectorSource(dados$hashtags))
...
wordcloud(d$words,d$freq,max.words=200,colors=brewer.pal(8,"Dark2"),scale=c(3,0.5),random.order=F)
