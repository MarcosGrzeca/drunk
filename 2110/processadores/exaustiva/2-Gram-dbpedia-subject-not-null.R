...
dados <- query("SELECT t.id,
       ...
       emoticonPos,
       emoticonNeg,
  ( SELECT GROUP_CONCAT(DISTINCT(REPLACE(subject, 'http://dbpedia.org/resource/Category:', '')))
   FROM
     ( SELECT c.resource AS resource,
      ...
      GROUP BY 1,
               2 ) AS louco
     JOIN resource_subject ty ON ty.resource = louco.resource
...
save(maFinal, file = "2110/rdas/2-Gram-dbpedia-subject-not-null.Rda")