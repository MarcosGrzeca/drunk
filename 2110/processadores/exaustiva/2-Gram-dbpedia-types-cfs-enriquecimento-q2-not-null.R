...
dados <- query("SELECT t.id,
  ...
  ( SELECT GROUP_CONCAT(DISTINCT(REPLACE(type, 'http://dbpedia.org/class/', '')))
   FROM
     ...
     AND ty.type IN ('http://dbpedia.org/class/yago/Agent114778436', 'http://dbpedia.org/class/yago/WikicatBeerStyles', 'http://dbpedia.org/ontology/Beverage', 'http://dbpedia.org/class/yago/Property104916342')
   ) AS entidadesDBPedia,
...
save(maFinal, file = "2110/rdas/2-Gram-dbpedia-types-enriquecimento-cfs-q2-not-null.Rda")